...
print("---- Beginning Simulation with Seed " + str(seed) + " and Correlation " + str(correlation) + " ----")
...
mask = stimulation_spike_trains[i] > (sim_time - 1000)
plt.scatter(stimulation_spike_trains[i][mask], i*np.ones((np.sum(mask))), alpha=0.2)
plt.show()
"""
...
print("---- Output Network Dynamics Simulated ---- Time: " + str(time.time() - start_time))
start_time = time.time()
