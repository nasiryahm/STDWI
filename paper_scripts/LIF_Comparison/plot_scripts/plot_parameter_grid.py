...
for correlation in correlations:
    ...
    for b_indx, batch_size in enumerate(akrout_batch_sizes):
        for d_indx, decay_value in enumerate(akrout_decay_values):
            ...
            if (akrout_map[b_indx, d_indx] + akrout_r_map[b_indx, d_indx])/2.0 > akrout_maxval:
                akrout_maxval = (akrout_map[b_indx, d_indx] + akrout_r_map[b_indx, d_indx])/2.0
                akrout_r_maxval = akrout_r_map[b_indx, d_indx]
                akrout_s_maxval = akrout_map[b_indx, d_indx]
    ...
    for f_indx, t_fast in enumerate(stdwi_taus):
        for s_indx, t_slow in enumerate(stdwi_taus[f_indx+1:]):
            ...
            weight_estimates = np.fromfile(filepath).reshape(-1, num_output_neurons, num_input_neurons)
            stdwi_map[f_indx, f_indx + s_indx + 1] = methods.sign_alignment(original_weight_matrix, weight_estimates[-1])
            stdwi_r_map[f_indx, f_indx + s_indx + 1] = np.corrcoef(original_weight_matrix.flatten(), weight_estimates[-1].flatten())[0,1]
            if ((stdwi_map[f_indx, f_indx + s_indx + 1] + stdwi_r_map[f_indx, f_indx + s_indx + 1])/2.0 > stdwi_maxval):
                stdwi_maxval = (stdwi_map[f_indx, f_indx + s_indx + 1] + stdwi_r_map[f_indx, f_indx + s_indx + 1])/2.0
                stdwi_r_maxval = stdwi_r_map[f_indx, f_indx + s_indx + 1]
                stdwi_s_maxval = stdwi_map[f_indx, f_indx + s_indx + 1]
                stdwi_argmax = (f_indx, f_indx + s_indx + 1)
    ...
    for a_indx, alpha in enumerate(rdd_alphas):
        for w_indx, window_size in enumerate(rdd_windows):
            ...
            if ((rdd_map[a_indx, w_indx] + rdd_r_map[a_indx, w_indx])/2.0 > rdd_maxval):
                rdd_maxval = (rdd_map[a_indx, w_indx] + rdd_r_map[a_indx, w_indx])/2.0
                rdd_r_maxval = rdd_r_map[a_indx, w_indx]
                rdd_s_maxval = rdd_map[a_indx, w_indx]
    ...
    print(akrout_s_maxval, stdwi_s_maxval, rdd_s_maxval)
